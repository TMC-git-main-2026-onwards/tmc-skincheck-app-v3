/*
 * TMC SkinCheck icon set - Lucide v0.453.0 (ISC licence), inlined so the
 * app has no runtime icon CDN dependency (works offline / under Capacitor).
 * Rendered via icon(name, size, style, label) -> inline SVG, stroke = currentColor.
 */
const TMC_ICONS={
...
function icon(name,size,style,label){
  const inner=TMC_ICONS[name]||'';
  const s=size||20;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${s}" height="${s}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"${style?` style="${style}"`:''}${label?` role="img" aria-label="${label}"`:' aria-hidden="true"'} class="icon">${inner}</svg>`;
}
if(typeof module!=='undefined'&&module.exports){module.exports={TMC_ICONS,icon};}
